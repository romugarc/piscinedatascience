...
def main():
    ...
    try:
        ...
        for column in testdf.columns:
            if column == 'knight':
                continue
            testdf[column] = (testdf[column] - testdf[column].min()) / (testdf[column].max() - testdf[column].min())
        for column in traindf.columns:
            if column == 'knight':
                continue
            traindf[column] = (traindf[column] - traindf[column].min()) / (traindf[column].max() - testdf[column].min())
        print(testdf)
